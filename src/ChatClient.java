...
public class ChatClient extends JFrame implements ActionListener {
    ...
    void connectServer() {
        try {
            socket = new Socket(txtServerIP.getText(), 8888); //port: 8888, use cmd "netstatus" to see whether this port occupied
    ...
    private void initLogin() throws IOException {
        ...
        if (strKey.equals("login")) {
            ...
            if (strStatus.equals("succeed")) {
                ...
                dialogLogin.dispose();
                new ClientThread(socket);  //if login successfully, open a new thread.
            ...
            if (obj.equals(btnLogin)) {
                if ((txtServerIP.getText().length() > 0)
                    ...
                    strSend = "login|" + txtName.getText() + "|"
                            + String.valueOf(txtPassword.getPassword());//trans user massage to server
                    out.println(strSend);
                    initLogin(); //send to server and then receive  from server to login
                } else {
                ...
                if (txtTalk.getText().length() > 0) {
                    out.println("talk|" + txtTalk.getText() + "|"
                            + txtName.getText() + "|"
                            + listOnline.getSelectedItem().toString());//talk to selected username
                    txtTalk.setText(""); //clean talk textfile
                }
            }
        } catch (Exception e) {
            System.out.println(e);
